    ...
    <el-card>
      ...
      <tree-table :data="cateList" :columns="columns" :selection-type="false" :expand-type="false" show-index index-text="#" border class="treeTable">
        ...
        <template slot="isok" slot-scope="scope">
          <i :class="['el-icon-success', 'iSuccessColor']" v-if="scope.row.cat_deleted === false"></i>
          <i :class="['el-icon-error', 'iErrorColor']" v-else></i>
        </template>
        <!-- 排序 -->
...
export default {
  ...
  methods: {
    ...
    // 选择项发生变化触发这个函数
    parentCateChanged () {
      // 如果 selectedKeys 数组中的 length 大于0，证明选中了父级分类，反之就说明没有选中任何父级分类
